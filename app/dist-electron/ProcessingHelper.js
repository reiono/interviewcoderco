...
class ProcessingHelper {
    ...
    async getAuthToken() {
        ...
        try {
            await this.waitForInitialization(mainWindow);
            const token = await mainWindow.webContents.executeJavaScript("window.__AUTH_TOKEN__");
            if (!token) {
    ...
    async processScreenshotsHelper(screenshots, signal) {
        try {
            ...
            try {
                ...
                const response = await axios_1.default.post(`${API_BASE_URL}/api/solve`, { imageDataList, language }, {
                    signal,
                    timeout: 300000,
    ...
    async processExtraScreenshotsHelper(screenshots, signal) {
        try {
            ...
            const response = await axios_1.default.post(`${API_BASE_URL}/api/debug`, { imageDataList, language }, {
                signal,
                timeout: 300000,
                validateStatus: function (status) {
                    return status < 500;
            ...
            if (error.response?.status === 401) {
                if (mainWindow) {
                    ...
                    await mainWindow.webContents.executeJavaScript("window.supabase?.auth?.signOut()");
                    mainWindow.webContents.send(this.deps.PROCESSING_EVENTS.DEBUG_ERROR, "Your session has expired. Please sign in again.");
                }
                return {
                    success: false,
            ...
            if (error.response?.data?.error === "No token provided") {
                if (mainWindow) {
                    mainWindow.webContents.send(this.deps.PROCESSING_EVENTS.DEBUG_ERROR, "Please sign in to continue.");
                }
                return {
                    success: false,
